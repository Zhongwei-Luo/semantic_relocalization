...
def dict_all_to_device(tensor_dict, device):
    ...
    for k in tensor_dict:
        if isinstance(tensor_dict[k], torch.Tensor):
            tensor_dict[k] = Variable(tensor_dict[k].type(torch.FloatTensor)).cuda()
...
def vis_voxel(vox_tensor_src, vox_tensor_ref):
    vol_src = to_numpy(vox_tensor_src)
    vol_ref = to_numpy(vox_tensor_ref)

    for i in range(vol_src.shape[0]):
        vox = vol_src[i,0,:,:,:]
        pcd = np.where(vox > 0.5)
        pcd = np.array(pcd)
        points_src = pcd / 16 - 1
        ...
        source_raw.points = o3d.utility.Vector3dVector(points_src)
        # source_raw.estimate_normals(
            # search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.5, max_nn=40))

        target_raw = o3d.geometry.PointCloud()
        target_raw.points = o3d.utility.Vector3dVector(points_ref )
        # target_raw.estimate_normals(
            # search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.5, max_nn=40))
        # target_raw1 = o3d.geometry.PointCloud()

        source_raw.paint_uniform_color([0.0, 0.0, 1])  # show A_pcd in blue
        target_raw.paint_uniform_color([1.0, 0.0, 0.0])  # show B_pcd in red
